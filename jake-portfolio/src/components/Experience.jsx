...
function TimelineEntry({ entry, index, isLast }) {
  return (
    <div className={`relative pl-10 ${!isLast ? 'pb-10' : ''} reveal reveal-delay-${Math.min(index + 1, 5)}`}>
      ...
      <div className="absolute left-0 top-1 w-9 h-9 rounded-xl flex items-center justify-center dark:bg-[#111] bg-gray-100 dark:border border dark:border-[#222] border-gray-200 dark:text-[#666] text-gray-500">
        {entry.icon}
      </div>

      <div className="p-5 rounded-2xl dark:bg-[#111] bg-white mono-border">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-3">
          <div>
            <h3 className="font-display font-700 text-base dark:text-white text-gray-900">
              {entry.role}
            </h3>
            <p className="text-sm font-medium dark:text-[#888] text-gray-500">
              {entry.org}
              <span className="dark:text-[#444] text-gray-400 font-normal"> · {entry.location}</span>
            </p>
          </div>
          <span className="flex-shrink-0 text-xs px-3 py-1 rounded-full dark:bg-[#0a0a0a] bg-gray-100 dark:text-[#555] text-gray-500 dark:border border dark:border-[#1e1e1e] border-gray-200">
            {entry.period}
          </span>
        </div>

        {entry.detail && (
          <p className="text-xs italic dark:text-[#555] text-gray-400 mb-3">{entry.detail}</p>
        )}
...
export default function Experience() {
  ...
  return (
    <section id="experience" ref={ref} className="py-24 md:py-32 dark:bg-[#0d0d0d] bg-gray-50">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12 lg:gap-16">
          ...
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-widest dark:text-[#444] text-gray-400 mb-8">
              Work &amp; Education
            </h3>
            <div>
          ...
          <div className="space-y-10">
            ...
            <div className="reveal reveal-delay-2">
              <h3 className="text-xs font-semibold uppercase tracking-widest dark:text-[#444] text-gray-400 mb-4 flex items-center gap-2">
                <Award size={12} />
                Awards &amp; Recognition
              </h3>
              <div className="space-y-2">
                {AWARDS.map((award) => (
                  <div
                    key={award.title}
                    className="flex items-start gap-3 p-3 rounded-xl dark:bg-[#111] bg-white dark:border border dark:border-[#1e1e1e] border-gray-200 hover:dark:border-[#2a2a2a] hover:border-gray-300 transition-colors"
                  >
                    <div className="w-8 h-8 rounded-lg dark:bg-[#1a1a1a] bg-gray-100 flex items-center justify-center flex-shrink-0">
                      <Award size={13} className="dark:text-[#666] text-gray-400" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-medium dark:text-[#aaa] text-gray-700 leading-snug">
                        {award.title}
                      </p>
                      <p className="text-[11px] dark:text-[#555] text-gray-400 mt-0.5">
                        {award.org} · <span className="dark:text-[#888] text-gray-600 font-medium">{award.amount}</span>
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                {EXTRA.map((item) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] dark:text-[#555] text-gray-400 mb-2">{item.org} · {item.period}</p>
                    {item.bullets.map((b, i) => (
                      <p key={i} className="text-xs dark:text-[#555] text-gray-400 leading-relaxed">{b}</p>
                    ))}
                  </div>
                ))}
              </div>
